...
const addMinutesToTime = (timeString: string, minutes: number): { time: string; nextDay: boolean } => {
...
const formatDateDisplay = (date?: Date, startTime?: string, endTime?: string, endDate?: string) => {
...
export function EnhancedCalendar({
  ...
  const handleDurationChange = (value: string) => {
    ...
    const { time: newEndTime, nextDay } = addMinutesToTime(startTime, durationMinutes)
  ...
  const handleStartTimeChange = (time: string) => {
    ...
    if (selectedDuration && selectedDuration !== "-1" && date) {
      ...
      const { time: newEndTime, nextDay } = addMinutesToTime(time, durationMinutes)
      onEndTimeChange?.(newEndTime)
